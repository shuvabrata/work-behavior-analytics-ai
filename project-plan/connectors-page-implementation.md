...
- [ ] **9. Create `app/api/connectors/v1/model.py`** — Pydantic models:
   - **Request models**:
     - `ConnectorConfigUpdateRequest`: `config: dict` — body for `PATCH /connectors/{type}` (connector-level JSONB config)
...
- [ ] **11. Create `app/api/connectors/v1/service.py`** — business logic:
    ...
    - `get_connector(db, connector_type)` — returns connector row with JSONB config
    - `update_connector_config(db, connector_type, config)` — validates and saves JSONB config
