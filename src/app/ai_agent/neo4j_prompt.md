...
**Source Control**: COLLABORATOR (has `permission`: READ/WRITE), BRANCH_OF, MODIFIES (has `additions`/`deletions`), REFERENCES
**Pull Requests & Commits**: INCLUDES, TARGETS, FROM, CREATED_BY (Commit or PullRequest → Person), REVIEWED_BY (has `state`), REQUESTED_REVIEWER, MERGED_BY, COMMENTED_ON
...
- COMMENTED_ON on Issues/Epics/Initiatives uses snapshot semantics: edges are replaced on each sync with aggregated `count` and timestamp properties
